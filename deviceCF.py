...
class CompactFlash(device):
	...
	def __init__(self, args, address, interrupt, debug):
		...
		if args.diskfile is not None:
			...
			if (self._file_size % CompactFlash.SECTOR_SIZE) != 0:
				raise RuntimeError('disk file {} size {} is not a multiple of the sector size'.format(args.diskfile, self._file_size))
	...
	def _command(self, command):
		if command == CompactFlash.CMD_READ_SECTORS:
			...
			raise RuntimeError('CF command {:02x} not supported'.format(command))

	def _trace_io(self, action):
		self.trace(action, 'count {} LBA {}'.format(self._r_sector_count, self._r_lba))
